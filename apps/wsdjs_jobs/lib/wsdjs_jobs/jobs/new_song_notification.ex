defmodule Wsdjs.Jobs.NewSongNotification do
  ...
  defp email_path("html"), do: Path.join(["#{:code.priv_dir(:wsdjs_jobs)}", "static", "email", "new_song_notification.html.eex"])
  defp email_path("txt"), do: Path.join(["#{:code.priv_dir(:wsdjs_jobs)}", "static", "email", "new_song_notification.text.eex"])
end
