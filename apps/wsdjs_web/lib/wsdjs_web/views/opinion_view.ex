defmodule WsdjsWeb.OpinionView do
  ...
  def options(kind, _conn, _song, opinions, nil) do
    ...
    default_options = [class: "song-opinion b-all-0 p-all-1 song-#{kind} tippy"]
  ...
  defp html_class(kind, %Wsdjs.Reactions.Opinion{kind: my_kind}) when kind == my_kind,
    do: "song-opinion b-all-0 p-all-1 song-#{kind} active tippy hvr-buzz-out"

  defp html_class(kind, _),
    do: "song-opinion b-all-0 p-all-1 song-#{kind} tippy hvr-buzz-out"
end
