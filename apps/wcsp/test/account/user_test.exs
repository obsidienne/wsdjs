defmodule Wcsp.UserTest do
  ...
  test "email must have a valid format" do
    assert {:email, {"has invalid format", [validation: :format]}} in errors_on(%User{}, %{email: "bullshit"})
  end

  test "email can't be blank" do
    assert {:email, {"can't be blank", [validation: :required]}} in errors_on(%User{}, %{email: nil})
  end
end
