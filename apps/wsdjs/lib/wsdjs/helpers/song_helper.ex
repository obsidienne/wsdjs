defmodule Wsdjs.SongHelper do
  ...
  def song_art_href(%Wcsp.Musics.Arts{cld_id: cld_id, version: version}) when is_binary(cld_id) do
    @base_url <> "v#{version}/" <> "#{cld_id}.jpg"
  end
  def song_art_href(nil), do: @missing_song_art

  def song_art_href_blured(%Wcsp.Musics.Arts{cld_id: cld_id, version: version}) when is_binary(cld_id) do
  ...
  def song_art_alt(%Wcsp.Musics.Songs{artist: artist, title: title}), do: "#{artist} - #{title} song art"

  def song_full_title(%Wcsp.Musics.Songs{artist: artist, title: title}), do: "#{artist}\u000A#{title}"
end
