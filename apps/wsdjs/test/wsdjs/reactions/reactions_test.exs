defmodule Wsdjs.ReactionsTest do
  ...
  describe "comments" do
    def comment_fixture() do
      ...
      {:ok, song} = Wsdjs.Musics.create_song(%{title: "my title#{System.unique_integer()}", artist: "my artist", genre: "soul", url: "http://youtu.be/dummy", user_id: user.id})
      {:ok, comment} = Reactions.create_comment(%{text: "mega song", user_id: user.id, song_id: song.id})
