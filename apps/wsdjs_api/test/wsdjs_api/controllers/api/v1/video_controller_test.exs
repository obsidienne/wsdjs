defmodule WsdjsApi.V1.VideoControllerTest do
  ...
  describe "create videos" do
    test "renders videos when data is valid", %{conn: conn} do
      ...
      resp = post conn, song_video_path(conn, :create, song.id), video: %{url: "http://www.youtube.com/toto"}
      assert %{"id" => id} = json_response(resp, 201)["data"]

      resp = get conn, song_video_path(conn, :index, song.id)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      ...
      conn = post conn, song_video_path(conn, :create, song.id), video: %{url: "dummy"}
