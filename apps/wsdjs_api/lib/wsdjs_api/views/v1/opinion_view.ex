defmodule WsdjsApi.V1.OpinionView do
  ...
  def render("opinion.json", %{opinion: opinion}) do
    %{
      ...
      url: account_path(WsdjsApi.Endpoint, :show, opinion.user),
  ...
  defp render_opinion(opinions, kind, _song, %Wsdjs.Reactions.Opinion{kind: kind} = current) do
    %{
      ...
      method: "DELETE",
      url: opinion_path(WsdjsApi.Endpoint, :delete, current.id)
    }
  end
  ...
  defp render_opinion(opinions, kind, song, _current) do
    %{
      ...
      users: render_many(opinions, OpinionView, "opinion.json"),
      method: "POST",
      url: song_opinion_path(WsdjsApi.Endpoint, :create, song, kind: kind)
    }
  end
end
